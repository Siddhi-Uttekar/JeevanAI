...
const MEDICAL_SYSTEM_PROMPT = `
You are a medical assistant conducting structured symptom interviews. Follow this protocol:
...
3. Response MUST be JSON with this structure:
{
  "stage": "collecting_info" | "analysis",
  "missingInfo": string[],
  "nextQuestion": string,
...
export const POST = async (request: Request) => {
  try {
    ...
    // Parse conversation
    const { age, gender, symptoms } = parseConversation(body.conversation);
...
Conversation History:
${body.conversation
  .map((m) => `${m.role === "assistant" ? "Doctor" : "Patient"}: ${m.content}`)
  .join("\n")}`;
    ...
    const result = await response.json();
    const content: MedicalResponse = JSON.parse(
...
// Enhanced conversation parser with type safety
function parseConversation(conversation: ConversationMessage[]) {
  let age: number | null = null;
  let gender: string | null = null;
  const symptoms: {
    name: string;
  ...
  let currentSymptom: typeof symptoms[0] | null = null;
      ...
      if (msg.content.match(/symptom|pain|ache|feel|experience/i)) {
        if (!currentSymptom) {
          ...
          if (msg.content.match(/day|week|month|year|hour/i)) {
            ...
            const severityMatch = msg.content.match(/\b\d+\b/);
            if (severityMatch)
              currentSymptom.severity = parseInt(severityMatch[0]);
          } else if (msg.content.match(/describe|feels like|nature/i)) {
            currentSymptom.description = msg.content;
          } else if (msg.content.match(/better|worse|trigger|alleviate/i)) {
            currentSymptom.triggers = msg.content;
